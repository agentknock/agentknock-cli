...
async fn uses_an_ssh_secret(key_type: &str, key_options: &[&str], test: SshCommandTest) {
    ...
    let probe = support::test_executable();
...
async fn requests_secret_use_and_executes_with_the_returned_environment() {
    ...
    let (relay_url, server) = websocket_server(move |listener| async move {
        ...
        if std::env::var_os("AGENTKNOCK_TEST_WITHOUT_PROCFS").is_some() {
            ...
            let launchers = plaintext["launcher_chain"].as_array().unwrap();
            assert!(
                !launchers.is_empty(),
                "launcher context must be retained when available"
            );
            assert_eq!(
                Path::new(launchers.last().unwrap().as_str().unwrap()),
                support::test_executable()
            );
        }
...
async fn replace_selected_native_file_after_approval() -> std::process::Output {
    ...
    fs::copy(support::test_executable(), &selected_path).unwrap();
