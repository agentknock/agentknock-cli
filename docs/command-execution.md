...
identifier and token, the SSH secret name, and those bytes. The helper also
uses `git` from its inherited `PATH` to collect advisory repository, branch,
and changed-path context. The changed paths come from the tree and first
parent named by the signing payload, not from mutable index or worktree state.
Failure to collect this context does not prevent signing. The service writes
an approved SSHSIG response to the signature file expected by Git. Every
